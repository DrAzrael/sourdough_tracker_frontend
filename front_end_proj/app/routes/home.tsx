...
export default function Home() {
  ...
  const cellSize = 100; // 100px x 100px cells
    ...
    for (let y = gridDimensions.bottom; y >= gridDimensions.top; y--) {
      for (let x = gridDimensions.left; x <= gridDimensions.right; x++) {
        ...
        cells.push(
          <div 
            key={`${x}-${y}`}
            className={`absolute border border-[#61401C] border-6 ${
            ...
            onDoubleClick={() => handleCellDoubleClick(x, y)}  // Changed to onDoubleClick
          >
  ...
  return (
    <>
      <div className="header h-[10%] bg-[#BA9042] flex items-center border-b-6 border-b-[#4a3015]">
  ...
  <div className="fixed inset-0 z-50">
    ...
    {/* Sidebar */}
    <div className="fixed right-0 top-0 h-full w-96 bg-[#A47A3B] shadow-lg transform transition-transform duration-300 ease-in-out">
      <div className="p-6 h-full flex flex-col">
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            className="text-white hover:text-[#F5DEAE] text-xl"
            onClick={() => setSelectedCell(null)}
          >
